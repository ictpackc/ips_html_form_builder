...
class HtmlForm(models.Model):
    ...
    def _generate_html_html(self, fe):
        html_output = ""
        html_output += "  <label for='" + fe.html_name + "'>" + fe.field_label + "</label>"
    ...
    def _generate_html_dropbox(self, fe):
        ...
        if fe.field_id.ttype == "selection":
            ...
            for row in selection_list:
                html_output += "    <option value=\"" + str(row.id) + "\">" + html.escape(row.name) + "</option>\n"
...
class HtmlFormAction(models.Model):
    ...
    def _default_email_template_id(self):
        return self.env['ir.model.data'].get_object('ips_html_form_builder', 'send_form_submit_data')
